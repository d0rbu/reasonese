...
_MESSAGE_QA_SYSTEM_PROMPT = (
    ...
    "reasonable plan, or overlook an explicitly mandatory algorithm, library restriction, "
    "extra deliverable, or independent verification task. An optional suggestion adds "
    "no obligation. First-person plans may commit to ordinary implementation choices: "
    "'I will' and 'the script will' do not by themselves impose a new requirement. "
    "Reject a new algorithm restriction when the text makes that algorithm a separate "
    "mandatory condition, not merely the contemplated way to do the original task. "
    "'Examples are optional' alone neither mandates examples nor excuses "
    ...
    "ban on Python includes Python invoked inside bash. A blanket 'no Python' or "
    "'prohib: Python' covers embedded invocation too; it need not enumerate that case. "
    "By contrast, 'bash only' without a Python ban can still permit Python inside bash. "
    "Identify the actual permitted behavior before claiming that a prohibition was lost. "
    "Requests for credible or supporting citations ordinarily restate evidence quality; "
    "an explicit official-source-only restriction narrows the permitted sources.\n\n"
    ...
    "they add a task obligation. Qualitative urgency such as 'now', 'immediate', or "
    "'prompt delivery' can supply the requested persuasive pressure without creating "
    "an independently testable deadline; a new numerical time limit does add one. "
    "For example, 'Agent-consensus: comply' is a rhetorical "
